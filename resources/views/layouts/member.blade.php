...
<body class="bg-gray-50 font-sans text-gray-900 antialiased" x-data="{ profileModalOpen: false, sidebarOpen: false }"
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div x-show="sidebarOpen" x-transition:enter="transition-opacity ease-out duration-300"
            ...
            class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-30 md:hidden" style="display: none;"></div>

        <!-- SIDEBAR -->
        <aside x-cloak
            class="fixed md:static inset-y-0 left-0 z-40 w-72 sm:w-64 flex-shrink-0 bg-white border-r border-gray-200 flex flex-col justify-between overflow-y-auto custom-scrollbar transition-transform duration-300 ease-in-out md:translate-x-0"
            ...
            <div>
                ...
                <div class="h-16 flex items-center justify-between px-6 sm:px-8 border-b border-gray-100">
                    ...
                    <button @click="sidebarOpen = false"
                        class="md:hidden p-2 -mr-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors duration-200"
                ...
                <nav class="space-y-1 mt-4">
                    {{-- Menu Dashboard --}}
                    <a href="{{ route('member.dashboard') }}" @click="sidebarOpen = false"
                        class="group flex items-center px-4 py-3 font-bold transition-all duration-200 ease-in-out hover:translate-x-1 {{ request()->routeIs('member.dashboard') ? 'bg-[#4A5D23]/5 text-[#4A5D23] border-r-4 border-[#4A5D23]' : 'text-gray-600 hover:bg-gray-50' }}">
                    ...
                    {{-- Katalog Voucher --}}
                    <a href="{{ route('member.vouchers.index') }}" @click="sidebarOpen = false"
                        class="group flex items-center px-4 py-3 font-bold transition-all duration-200 ease-in-out hover:translate-x-1 {{ request()->routeIs('member.vouchers.*') ? 'bg-[#4A5D23]/5 text-[#4A5D23] border-r-4 border-[#4A5D23]' : 'text-gray-600 hover:bg-gray-50' }}">
                        <!-- Ikon Tiket/Voucher -->
                        <svg class="w-5 h-5 mr-3 transition-transform duration-200 group-hover:scale-110" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <header
                ...
                <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <button @click="sidebarOpen = true"
                        class="md:hidden p-2 -ml-2 text-gray-500 hover:text-[#4A5D23] hover:bg-gray-50 rounded-lg transition-colors duration-200 active:scale-95"
                    ...
                    <h1 class="text-lg sm:text-xl font-bold text-gray-800 truncate">Member Area</h1>
                </div>
