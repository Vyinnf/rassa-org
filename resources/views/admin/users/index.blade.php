            ...
            <tbody class="divide-y divide-gray-100 text-gray-700">
                ...
                @forelse ($users as $user)
                    <tr class="hover:bg-gray-50/80 transition">
                        ...
                        <td class="px-6 py-4">
                            @if($user->role === 'admin')
                                <span class="px-3 py-1 bg-[#4A5D23]/10 text-[#4A5D23] rounded-full text-xs font-bold tracking-wide">Administrator</span>
                            @else
                                <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-bold tracking-wide">Pengunjung</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            @if(auth()->id() === $user->id)
                                <span class="text-gray-400 font-medium italic text-xs px-3 py-1 bg-gray-50 rounded-lg">Akun Anda</span>
                            @else
                                ...
                                @if($user->role === 'customer')
                                    <form action="{{ route('admin.users.add-points', $user->id) }}" method="POST" class="mb-2">
                                        ...
                                        <div class="flex items-center justify-end space-x-2">
                                            <input type="number" name="points" placeholder="50" class="w-16 px-2 py-1 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-[#4A5D23] outline-none" required>
                                            <button type="submit" class="text-xs bg-[#4A5D23] text-white px-2 py-1 rounded-lg hover:bg-[#3b4b1c] transition font-bold">+ Poin</button>
                                        </div>
                                    </form>
                                @endif
                                ...
                                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 font-bold transition px-3 py-1 hover:bg-red-50 rounded-lg text-xs" onclick="return confirm('Peringatan: Hapus akun ini?')">Hapus Akses</button>
                                </form>
                            @endif
                        </td>
                    </tr>
