...
@section('content')
    ...
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('admin.menus.create') }}" class="px-5 py-2.5 bg-[#4A5D23] text-white text-sm font-semibold rounded-xl hover:bg-[#3b4b1c] transition shadow-md flex items-center">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            Tambah Menu
        </a>
    </div>

    <!-- Tabel High-Contrast & Rounded -->
                    ...
                    <tr class="hover:bg-gray-50/80 transition">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                @if($menu->image)
                                    <img src="{{ asset('storage/' . $menu->image) }}" alt="{{ $menu->name }}" class="w-10 h-10 rounded-lg object-cover border border-gray-200 mr-3">
                                @else
                                    <div class="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center mr-3 text-gray-400">
                        ...
                        <td class="px-6 py-4">
                            @if($menu->is_available)
                                <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold tracking-wide">
                                    Tersedia
                                </span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right space-x-3">
                            <a href="{{ route('admin.menus.edit', $menu->id) }}" class="text-blue-600 hover:text-blue-800 font-bold transition">Edit</a>
                            <form action="{{ route('admin.menus.destroy', $menu->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-800 font-bold transition" onclick="return confirm('Yakin ingin menghapus menu ini secara permanen?')">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @empty
