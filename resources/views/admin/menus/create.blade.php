...
<div class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] overflow-hidden max-w-3xl">
    <form action="{{ route('admin.menus.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
        ...
        <p class="text-xs font-medium text-red-500 mb-6 italic">
            Tanda bintang (<span class="text-red-500 font-bold">*</span>) menunjukkan kolom yang wajib diisi.
        </p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                <label for="name" class="block text-sm font-bold text-gray-800 mb-2">
                    Nama Produk <span class="text-red-500" title="Wajib diisi">*</span>
                </label>
                <input type="text" name="name" id="name" required value="{{ old('name') }}"
                    placeholder="Contoh: Es Kopi Susu Rassa"
            ...
            <!-- Input Kategori -->
            <div>
                <label for="category" class="block text-sm font-bold text-gray-800 mb-2">
                    Kategori <span class="text-red-500" title="Wajib diisi">*</span>
                </label>
                <select name="category" id="category" required
                    ...
                    <option value="Non-Coffee" {{ old('category') == 'Non-Coffee' ? 'selected' : '' }}>Non-Coffee
                    </option>
                    <option value="Snack" {{ old('category') == 'Snack' ? 'selected' : '' }}>Snack</option>
                    <option value="Main Course" {{ old('category') == 'Main Course' ? 'selected' : '' }}>Main Course
                    </option>
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                <label for="price" class="block text-sm font-bold text-gray-800 mb-2">
                    Harga (Rp) <span class="text-red-500" title="Wajib diisi">*</span>
                </label>
            ...
            <!-- Input Status -->
            <div>
                <label for="is_available" class="block text-sm font-bold text-gray-800 mb-2">
                    Status Ketersediaan <span class="text-red-500" title="Wajib diisi">*</span>
                </label>
        ...
        <!-- Input Deskripsi -->
        <div class="mb-6">
            <label for="description" class="block text-sm font-bold text-gray-800 mb-2">Deskripsi Singkat
                (Opsional)</label>
            <textarea name="description" id="description" rows="3" placeholder="Tambahkan deskripsi jika diperlukan..."
